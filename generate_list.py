...
B6-12 mice based on the start and end date
...
files = [file for file in os.listdir(HS_BASE)
         if (os.path.isfile(os.path.join(HS_BASE, file)) and
             file.endswith('_video.avi'))]

# file time
file_time = []
for file in files:
    date_object = datetime.strptime('20'+file[:13], '%Y%m%d_%H%M%S')
    file_time.append(date_object)

date_start = datetime.strptime('20230904', '%Y%m%d')
date_end = datetime.strptime('20231019', '%Y%m%d')
